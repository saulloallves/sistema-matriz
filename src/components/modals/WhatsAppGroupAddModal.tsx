...
import { WhatsAppGroupInsert, WhatsAppGroupKind, whatsappGroupKindLabels } from '@/types/whatsapp';
...
interface WhatsAppGroupAddModalProps {
  open: boolean;
  onClose: () => void;
}

export const WhatsAppGroupAddModal: React.FC<WhatsAppGroupAddModalProps> = ({
  open,
  onClose
}) => {
  const { createGroup, isCreating } = useWhatsAppGroups();
  ...
  const onSubmit = async (data: GroupFormData) => {
    try {
      await createGroup(data as WhatsAppGroupInsert);
      handleClose();
    } catch (error) {
      ...
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display="flex" alignItems="center" gap={1}>
            ...
            <Typography variant="h6">Adicionar Grupo WhatsApp</Typography>
          </Box>
