...
import { Edit, Webhook } from 'lucide-react';
import { WebhookSubscription } from '@/hooks/useWebhookSubscriptions';

interface WebhookEditModalProps {
  open: boolean;
  onClose: () => void;
  webhook: WebhookSubscription | null;
  onSave: (id: string, updates: Partial<WebhookSubscription>) => void;
  isLoading?: boolean;
}
      ...
      <DialogContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 2 }}>
          ...
          >
            ...
            <br />
            <Typography variant="caption" color="text.secondary">
              Criado: {new Date(webhook.created_at).toLocaleString('pt-BR')}
            </Typography>
          </Box>
        </Box>
      </DialogContent>
