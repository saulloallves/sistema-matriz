...
import { useFranqueadosFilhos, FranqueadoFilho } from '@/hooks/useFranqueadosFilhos';
import { format } from 'date-fns';

interface ActionCellProps {
  row: FranqueadoFilho;
  onView: (filho: FranqueadoFilho) => void;
  onEdit: (filho: FranqueadoFilho) => void;
  onDelete: (filho: FranqueadoFilho) => void;
}
...
export default function FranqueadosFilhosPage() {
  const { franqueadosFilhos, isLoading, deleteFranqueadoFilho } = useFranqueadosFilhos();
  const [viewModalOpen, setViewModalOpen] = useState(false);
  const [editModalOpen, setEditModalOpen] = useState(false);
  const [addModalOpen, setAddModalOpen] = useState(false);
  const [selectedFilho, setSelectedFilho] = useState<FranqueadoFilho | null>(null);

  const statsCards = useMemo(() => (
    <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
      <Card sx={{ flex: 1 }}>
        <CardContent sx={{ py: 2 }}>
          <Typography variant="h5">{franqueadosFilhos.length}</Typography>
      ...
      <Card sx={{ flex: 1 }}>
        <CardContent sx={{ py: 2 }}>
          <Typography variant="h5">
            {franqueadosFilhos.filter(f => f.gender === 'Masculino').length}
          </Typography>
      ...
      <Card sx={{ flex: 1 }}>
        <CardContent sx={{ py: 2 }}>
          <Typography variant="h5">
            {franqueadosFilhos.filter(f => f.gender === 'Feminino').length}
          </Typography>
  ...
  const handleView = (filho: FranqueadoFilho) => {
    setSelectedFilho(filho);
    setViewModalOpen(true);
  };

  const handleEdit = (filho: FranqueadoFilho) => {
    setSelectedFilho(filho);
    setEditModalOpen(true);
  };

  const handleDelete = (filho: FranqueadoFilho) => {
    if (window.confirm(`Tem certeza que deseja remover ${filho.name}?`)) {
      deleteFranqueadoFilho(filho.id);
    }
  };
