...
export default function EditFranchiseePage() {
  ...
  const handleIdentifierSubmit = async () => {
    ...
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('get-franchisee-data-for-edit', {
        body: { identifier },
      });

      if (error || !data.found) {
        toast.error("Nenhum franqueado encontrado. Verifique o documento ou cadastre-se.", { duration: 5000 });
        setTimeout(() => { window.location.href = 'https://cadastro.girabot.com.br'; }, 5000);
        return;
      }
      ...
      setActiveStep(1);
      toast.success("Franqueado encontrado! Prossiga com a verificação de segurança.");
  ...
  const handleRequestOtp = async (type: 'email' | 'phone') => {
    ...
    try {
      ...
      const target = type === 'phone' ? franchiseeData.contact : franchiseeData.email;

      if (!target) {
        toast.error(`Não foi possível encontrar um ${type === 'phone' ? 'telefone' : 'e-mail'} para verificação.`);
        setLoading(false);
        return;
      }

      const { error } = await supabase.auth.signInWithOtp(type === 'phone' ? { phone: target } : { email: target });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, entity: 'franchisee' | 'unit', index?: number) => {
      ...
      case 0:
        return (
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Typography sx={{ mb: 3 }}>Para começar, informe seu CPF/RNM ou o CNPJ da sua unidade.</Typography>
            <TextField label="CPF/RNM ou CNPJ" value={identifier} onChange={(e) => setIdentifier(e.target.value)} fullWidth sx={{ mb: 2 }} />
            <Button onClick={handleIdentifierSubmit} variant="contained" disabled={loading}>
      ...
      case 2:
        return (
          <form onSubmit={handleSubmit}>
            ...
            <Button type="submit" variant="contained" sx={{ mt: 4 }} disabled={loading}>
              {loading ? <CircularProgress size={24} /> : 'Enviar Alterações para Aprovação'}
            </Button>
          </form>
        );
      default:
  ...
  return (
    <>
      ...
      <AppBar position="static"><Toolbar><Typography variant="h6">Editar Dados Cadastrais</Typography></Toolbar></AppBar>
